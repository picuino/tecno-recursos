
.. include:: freecad-subs.rst

.. _freecad-visualizacion:

Visualización de piezas
=======================
En este tutorial vamos a cambiar las propiedades de visualización de
una pieza en tres dimensiones.

|br|

1. Abrimos la aplicación **FreeCAD** para Windows, que
   tiene el siguiente icono.

   .. image:: freecad/_images/freecad-icono.png


#. A continuación hacemos clic en el icono para crear un
   **nuevo documento** |icono-nuevo-documento| o bien
   seleccionamos en el menú ``Archivo... Nuevo``.

   Se abrirá una nueva pestaña con un documento vacío, donde
   podemos comenzar a diseñar.

   |br|

#. Seleccionamos el **banco de trabajo Part** para
   comenzar a diseñar objetos en 3 dimensiones.

   .. image:: freecad/_images/freecad-banco-part.png

   |br|

#. Ahora **creamos un cubo** clicando el primer icono de la barra
   de objetos sólidos.

   .. image:: freecad/_images/freecad-barra-solidos.png

   |br|

#. Cambiamos el nombre del cubo clicando con el botón derecho del
   ratón y seleccionando renombrar o bien clicando con el botón
   izquierdo del ratón y pulsando la tecla **F2**.

   .. image:: freecad/_images/freecad-renombrar-cubo.png

   Cambiamos el nombre "Cubo" por el nuevo nombre **"Mi_cubo"**

   .. image:: freecad/_images/freecad-cubo-renombrado.png

   |br|

#. Ahora, presionando la **tecla espacio**
   el cubo desaparecerá y aparecerá en pantalla.

   Cuando el cubo es invisible, su nombre cambia a color gris.

   .. image:: freecad/_images/freecad-cubo-invisible.png

   Con esta función podemos ver las piezas ocultas que están detrás
   de un objeto sólido.

   |br|

#. Otra forma de poder ver el interior de una pieza y ver a través
   es cambiar la propiedad de transparencia, por ejemplo al 50%.

   La propiedad de **Transparencia** está en la pestaña **Vista**.

   .. image:: freecad/_images/freecad-pestania-vista.png

   Si ahora añadimos un **Cilindro** al diseño, podremos ver
   la nueva pieza a través del cubo.

   .. image:: freecad/_images/freecad-cubo-transparente.png

   |br|

#. Además de la transparencia, en la pestaña Vista se pueden cambiar
   otras propiedades tales como

   el color de las caras de la pieza (**Shape Color**),

   el color de las aristas (**Line Color**),

   y el color de los vértices (**Point Color**).

   Una propiedad útil es cambiar el tamaño de los puntos (Point Size)
   para que sea más sencillo seleccionarlos.


-----


Ejercicios
----------

1. Abre FreeCAD y crea un nuevo documento con un cubo y un cilindro.
   Cambia las propiedades del cubo para que sea transparente al 50%
   con las caras de color azul y las aristas de color amarillo.

   .. image:: freecad/_images/freecad-cubo-transparente-colores.png

   |br|


-----

Videotutorial
-------------

`Propiedades de visualización.
<https://www.youtube.com/embed/dOdAtUmgW4k>`__